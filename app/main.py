...
async def query_vehicles(query: VehicleQuery):
    """Query vehicles by various criteria"""
...
async def query_empresas(query: EmpresaQuery):
    """Query empresas by various criteria"""
    async with get_db() as db:
        # Build query dynamically based on provided filters
        conditions = []
        params = {}
...
async def query_registrations(query: RegistrationQuery):
    """Query registrations by various criteria"""
    async with get_db() as db:
        # Build query dynamically based on provided filters
        conditions = []
        params = {}
        ...
        if query.uf_emplacamento:
            conditions.append("uf_emplacamento = :uf")
            params['uf'] = query.uf_emplacamento
        
        if query.preco_min:
            conditions.append("preco >= :preco_min")
            params['preco_min'] = query.preco_min
        
        if query.preco_max:
            conditions.append("preco <= :preco_max")
            params['preco_max'] = query.preco_max
        
        if query.preco_validado:
            conditions.append("preco_validado = :preco_validado")
            params['preco_validado'] = query.preco_validado
